function Game() {
	...
	this.isOver = function() {
		// This function states when the game is over (for agent use)
...
// 		turn = 1;
// 		doublecount = 0;
// 		// ... any other game variables that need to be reset

// 		// player = [
// 		//     null, // The first index is not used
// 		//     new Player("AI Player 1", "red"), // Replace "red" with the actual color
// 		//     new Player("AI Player 2", "blue") // Replace "blue" with the actual color
// 		// ];
		
// 		// Assign AI to the players
...
// 			getState(p, s);
// 		}
// 		// pcount = player.length - 1; // Update the player count

// 		// Reset player positions
// 		for (turn = 1; turn <= pcount; turn++) {
// 			if (player[turn]) {
// 				player[turn].position = 0;
// 				player[turn].money = 1500;
// 				player[turn].creditor = -1;
// 				player[turn].jail = false;
// 				player[turn].jailroll = 0;
// 				player[turn].communityChestJailCard = false;
// 				player[turn].chanceJailCard = false;
// 				player[turn].bidding = true;
// 				player[turn].index = turn;
// 			}
// 		}
...
// 			square[index].house = 0;
// 			square[index].hotel = 0;
// 			square[index].landcount = 0;
// 		}

// 		// Clear alerts
// 		document.getElementById("alert").innerHTML = "";
// 		//addAlert("It's " + player[turn].name + "'s turn");

// 		// Reset UI elements
// 		$("#control").show();
// 		$("#board").show();
// 		$("#refresh").hide();
// 		document.getElementById("stats").style.width = "900px";

// 		// Start a new turn
// 		if (player[turn]) { // Check if player[turn] is defined before calling play
// 			play();
// 		} else {
// 			console.error("Player " + turn + " is undefined.");
// 		}
// }

	this.reset = function () {
		// Store the original player count when the game starts
		var originalPcount = 2 + 1;
		// Reset game variables...
		turn = 1;
		doublecount = 0;
		
		// Initialize players
		player = [null]; // The first index is not used
		for (var i = 1; i <= originalPcount; i++) {
			var p = new Player("AI Player " + i, "red"); // Create new player
			p.human = false;
			p.AI = new AITest(p);
			getState(p, s);
	...
	for (var i = 1; i <= originalPcount; i++) {
		var playerIndex = i;
		if (playerIndex > pcount) {
			playerIndex -= pcount;//
		}

		if (player[playerIndex]) {
			player[playerIndex].position = 0;
			player[playerIndex].money = 1500;
			player[playerIndex].creditor = -1;
			player[playerIndex].jail = false;
			player[playerIndex].jailroll = 0;
			player[playerIndex].communityChestJailCard = false;
			player[playerIndex].chanceJailCard = false;
			player[playerIndex].bidding = true;
	...
	this.next = function() {
		...
		if (pcount > 1) {
			if (!p.human && p.money < 0) {
				// p.AI.payDebt();
				p.AI.performAction({type: 'payDebt'});

				if (p.money < 0) {
					// Replace the popup function call with a console.log
					console.log(p.name + " is bankrupt. All of its assets will be turned over to " + player[p.creditor].name + ".");
					game.bankruptcy();
				} else {
					roll();
				}
	...
	this.auction = function() {
		...
		if (this.isOver()) {
			console.log("Game has ended. Auction will be stopped.");
			return false;
		}
		...
		if (!player[currentbidder].human ) {
			...
			if (bid === -1) {
				...
				// If the AI chose to pass, call the auctionPass function
				this.auctionPass();
			} else {
				// If the AI placed a bid, call the auctionBid function
				this.auctionBid(bid);
			}
		} else {
			// If the current bidder is a human, show the popup
			// ...
	...
	this.auctionPass = function() {
		...
		while (true) {
			...
			if (currentbidder == highestbidder) {
				...
				if (!p.human && pcount > 0 ) {
					var bid = p.AI.performAction({ type: "bid", propertyId: auctionproperty, currentBid: highestbid });

					if (bid === -1 || highestbid >= p.money) {
						p.bidding = false;
						console.log(p.name + " exited the auction.");
						continue;
						...
						this.auctionBid(bid);
						console.log(p.name + " bid $" + bid + ".");
						// Don't call this.auctionPass() here
						return;
					}
				} else {
					break;
	...
	this.auctionBid = function(bid) {
		if (player[currentbidder].human) {
			...
			// Handle AI bidding...
			var aiBid = player[currentbidder].AI.performAction({type: 'bid', currentBid: highestbid, propertyId: auctionproperty});
			if (aiBid > highestbid && aiBid <= player[currentbidder].money) {
				highestbid = aiBid;
				highestbidder = currentbidder;
				this.auctionPass();
				}
	...
	this.eliminatePlayer = function() {
		...
		if (pcount === 1) {
			...
			popup('<button type="button" onclick="window.location.reload()">Refresh Page</button>');
...
function updateMoney() {
	...
	if (p !== null) { // Check if the player is not null
		...
		for (var i = 1; i <= pcount; i++) {
			...
			if (p_i !== null) { // Check if the player is not null
				$("#moneybarrow" + i).show();
...
function buy() {
	...
	if (p.money >= cost) {
		...
		console.log(p.name + " bought " + property.name + " for " + property.pricetext + ".!!");
		...
		if(p.human){
			...
			console.log("<p>" + p.name + ", you need $" + (property.price - p.money) + " more to buy " + property.name + ".</p>"
			)
		}
		//popup("<p>" + p.name + ", you need $" + (property.price - p.money) + " more to buy " + property.name + ".</p>");
	}
...
function land(increasedRent) {
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			if (p.AI.performAction({ type: "buyProperty", propertyId: p.position })) {
				console.log('AI decided to buy property');
				buy();
			}
		} else {
...
function play() {
	...
	if (!p.human) {
		if (!p.AI.performAction({ type: "beforeTurn" })) {
			p.AI.performAction({type: "END_TURN"});
		}
	}
}
